...
class ListingManager(BaseService):
    ...
    async def create(self, event_data):
        ...
        try:
            cross_listing = await self.adaptor.create_soap_listing(self.listing_data, self.product_data, cross_listing, user)
    ...
    async def deactivate(self, event_data):
        ...
        try:
            ...
            res = await self.adaptor.deactivate_listing(cross_listing, user)
        print(res)
        if 'warnings' in res or res['listingId'] == cross_listing.secondary_external_id:
